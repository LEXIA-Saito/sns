...
- ✅ **レベル・経験値バー** — 投稿を重ねると上がる。名前の横にアイコンとレベルが出る
  - 文字だけの投稿 +10 / 画像・動画つきの投稿 +25
  - 投稿ポップアップ内に**シンプルな横線のプログレスバー**を表示。現在の進捗と、投稿後の伸び代を分かりやすくプレビュー
...
- ✅ **黒（白線: whiteline）テーマが本番標準** — 背景 `#080808` に白い枠線、白抜きロゴ、ビットマップ書体（PixelMplus12）
  - 配色・フォント設定は `src/app/theme.css` と `src/app/theme-meta.ts` のCSS変数で管理
  - かつて存在したテスト用の他色プレビュー画面（`/themes`）およびランタイム切替パラメータ（`?theme=...`, `?preview=1`）は**完全に廃止**
- ✅ **Lucideアイコン** 使用

## 機能エントリ(URL)

公開ルートは以下の2つのみです（他色テスト用URL `/themes` は廃止されました）。

| パス | 説明 |
|------|------|
| `/` | メインフィード(投稿一覧・投稿フォーム・コメント) |
| `/status` | データベース接続状態確認ページ（運営用カードログイン時のみ表示） |
...
外部Firebaseサービスに依存しない高速な単体テストを備えています。
...
2. ポップアップ内で現在のレベルと横線プログレスバーを確認
3. メッセージを入力、必要なら画像・動画を追加（投稿後の見込み経験値がバー上でプレビュー表示されます）
4. 「投稿する」で完了 → 全員の画面に即時反映
...
必要な累計経験値は 0 / 30 / 80 / 150 / 250 / 400 / 600 / 850 / 1150 / 1500(以降は400ずつ)。
アイコンは 芽(Lv1-2) → 若木(Lv3-4) → 炎(Lv5-6) → 星(Lv7-8) → 冠(Lv9-) と変わる。
