...
26アカデミーの例会で使用する専用SNSアプリです。PR動画・画像・意気込みをアカデミーメンバーが投稿し、LOMメンバーからは思い出や激励コメントをリアルタイムで投稿できます。例会中はスクリーンに表示しながら、参加者がその場でコメントを投稿できます。

## プロジェクト概要

- **名称**: 26アカデミー 例会SNS
- **目的**: 例会をリアルタイムで盛り上げる投稿・コメント共有プラットフォーム
- **メイン利用環境**: スマートフォン(PC・スクリーン表示にも対応)
...
- ✅ **スクリーン表示モード** (`/screen`) — 例会の大画面用。投稿をグリッド表示し自動スクロール
...
| `/screen` | 例会スクリーン表示モード(大画面・自動スクロール) |
...
### スクリーン表示(例会運営者向け)
- 右上の「スクリーン表示」から `/screen` へ
- 大画面に投稿が自動スクロールで流れます(マウスを乗せると一時停止)
