...
import type { Post, AppSettings, AccountActivity } from "../src/lib/types";

test("受付状態・締切判定の検証 (canCreatePost)", async (t) => {
  ...
  await t.test("スイッチOFF時は一般メンバーは投稿不可", () => {
  ...
  await t.test("締切日時前の境界値では受付中", () => {
    const st: AppSettings = { postDeadline: deadline };
...
test("モデレーションと表示判定の検証 (isPostVisible)", async (t) => {
...
test("アカウント進捗集計とCSV出力の検証", async (t) => {
  ...
  await t.test("実績（非表示含む投稿数、写真投稿数）が集計される", () => {
    ...
    assert.equal(user2.postCount, 0);
  });
